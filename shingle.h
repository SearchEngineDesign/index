...
const float SIMILARITY_THRESHOLD = 0.8;
constexpr int NUM_SIMILARITY = SIMILARITY_THRESHOLD * SIGNATURE_SIZE;

//just gettin my shit down to a
 const size_t p = 255;
...
class ShingleMap {
    ...
    */
    ...
    void addDocument(char (&signature)[SIGNATURE_SIZE]);
    /*
    ...
    */
    ...
    bool isSimilar(char (&signature)[SIGNATURE_SIZE]);

    private:
    std::unordered_map<size_t, vector<size_t>> bandTables[NUM_BANDS];
    vector<char[SIGNATURE_SIZE]> shingles_signatures;
    ...
    void sign(const vector<size_t> &shingles, char (&signature)[SIGNATURE_SIZE]);
